...
class Project_model extends CI_Model
{
    ...
    {
        $method = $this->router->fetch_method();
//         SELECT `BaseTbl`.`projectId`, `BaseTbl`.`userID`, `BaseTbl`.`sales`, `BaseTbl`.`is_priority`, `BaseTbl`.`projectName`, `BaseTbl`.`scope`, `BaseTbl`.`dueDate`, `BaseTbl`.`estStartDate`, `BaseTbl`.`rom`, `BaseTbl`.`dueTime`, `BaseTbl`.`jobWalkTime`, `BaseTbl`.`address`, `BaseTbl`.`estimator`, `BaseTbl`.`mainContact`, `BaseTbl`.`createdDtm`, `BaseTbl`.`salesRepId`, `BaseTbl`.`stageId`, `BaseTbl`.`bid_price`, `BaseTbl`.`contract`, `BaseTbl`.`company`, `User`.`name`, `Stage`.`stageName`, `BaseTbl`.`phoneNo1`, `BaseTbl`.`email`, `BaseTbl`.`updatedDtm`, `BaseTbl`.`filesystem_id`, `BaseTbl`.`notes`, `jobtype`.`jobType`, `jobtype`.`color`, `pcl`.`clientID`, `cl`.`client_name` as `client_company`, `cnt`.`contact_name`, `cnt`.`contact_phone`, `cnt`.`contact_phone` FROM `amp_projects` as `BaseTbl` LEFT JOIN `amp_users` as `User` ON `User`.`userId` = `BaseTbl`.`sales` LEFT JOIN `amp_stages` as `Stage` ON `Stage`.`stageId` = `BaseTbl`.`stageId` LEFT JOIN `amp_jobtype` as `jobtype` ON `jobtype`.`id` = `BaseTbl`.`jobtypeid` LEFT JOIN `amp_project_clients` as `pcl` ON `pcl`.`projectId` = `BaseTbl`.`projectId` LEFT JOIN `amp_clients` as `cl` ON `cl`.`id` = `pcl`.`clientId` 
// LEFT JOIN amp_project_contact as pc ON pc.projectId = BaseTbl.projectId
// LEFT JOIN `amp_contact` as `cnt` ON `pc`.`contactId` = `cnt`.`id` WHERE `BaseTbl`.`isDeleted` = 0 AND `Stage`.`stageId` = 11 GROUP BY `BaseTbl`.`projectId` ORDER BY `BaseTbl`.`projectId` DESC
        
        $this->db->select('BaseTbl.projectId,BaseTbl.userID,BaseTbl.sales,BaseTbl.is_priority,BaseTbl.projectName,BaseTbl.scope,BaseTbl.dueDate,BaseTbl.estStartDate,BaseTbl.rom,BaseTbl.dueTime,BaseTbl.jobWalkTime,BaseTbl.address,BaseTbl.estimator,BaseTbl.mainContact, BaseTbl.createdDtm, BaseTbl.salesRepId, BaseTbl.stageId, BaseTbl.bid_price,BaseTbl.contract,BaseTbl.company,User.name,Stage.stageName,BaseTbl.phoneNo1,BaseTbl.email,BaseTbl.updatedDtm,BaseTbl.filesystem_id,BaseTbl.notes,jobtype.jobType,jobtype.color,pcl.clientID,cl.client_name as client_company,cnt.contact_name,cnt.contact_phone,cnt.contact_phone,acounty.county');
        $this->db->from('amp_projects as BaseTbl');
        $this->db->join('amp_users as User', 'User.userId = BaseTbl.sales','left');
        $this->db->join('amp_stages as Stage', 'Stage.stageId = BaseTbl.stageId','left');
        $this->db->join('amp_jobtype as jobtype', 'jobtype.id = BaseTbl.jobtypeid','left');
               $this->db->join('amp_project_clients as pcl', 'pcl.projectId = BaseTbl.projectId','left');
        $this->db->join('amp_clients as cl', 'cl.id = pcl.clientId','left');
        $this->db->join('amp_project_contact as pc', 'pc.projectId = BaseTbl.projectId','left');
        $this->db->join('amp_contact as cnt', 'pc.contactId = cnt.id','left');
     ...
     * @return number $insert_id : This is last inserted id
     */
     function editBid($bidArray)
    {
        $this->db->where('id ' , $bidArray['bidId']);
        $this->db->set("companyName", $bidArray['companyName']);
        $this->db->set("bidPrice", $bidArray['bidPrice']);
     ...
     * @return number $insert_id : This is last inserted id
     */
     function editContractingBid($bidArray)
    {
        $this->db->where('projectId ' , $bidArray['projectId']);
        $this->db->set("bid_price", $bidArray['bidPrice']);
        $this->db->update('amp_projects');
        return true;
    }
    ...
    function updateBidsOnAdd($projectId,$bidArray, $userId,$bidRandomIDVal,$buildingSf){
        ...
        for($i=0;$i<=count($bidArray)-1;$i++){
            ...
            if($buildingSf > 0 || $buildingSf !=""){
                ...
                $this->db->set("unitCost", "bidPrice/".$buildingSf, FALSE); 
            }
    ...
    function getBidByProjectIdForView($projectId)
    {
        if($projectId != ""){
            $this->db->select('*');
            //$this->db->where('userId' , $userId);
  ...
  return false;
    ...
    function editNotes($notesArray){
        $this->db->where('id', $notesArray['notesId']);
        
        $this->db->set("notes", $notesArray['notes']); 
        $this->db->set("updatedDtm", date('Y-m-d H:i:s')); 
        return $this->db->update('amp_notes_history');
    }
    ...
    function graphTotalWonSalesQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)) as firstsum FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ";
        return $this->db->query($sql)->row_array();
    }
    
    function graphTotal90WonSalesQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)) as firstsum FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 13 and estimator > 0 ";
        return $this->db->query($sql)->row_array();
    }


    function graphProjectPriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,dueDate FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate'  ";
        return $this->db->query($sql)->result_array();
    }

    function graphCompanyPriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (SELECT (IFNULL(sum(bid_price),0)/1000000) from amp_projects where company like '%north%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as northsum,
            (SELECT (IFNULL(sum(bid_price),0)/1000000) from amp_projects where company like '%contractor%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as contractsum,
            (SELECT count(*) from amp_projects where company like '%north%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as north,
            (SELECT count(*) from amp_projects where company like '%contractor%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as contract 
             FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' limit 1 ";
        return $this->db->query($sql)->result_array();
    }  

    function graphEstimatorPriceQuantityDaily($startDate = '',$endDate = ''){        
        $sql = " SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,amp_users.name,amp_users.userId FROM `amp_projects` left join amp_users on amp_users.userId = amp_projects.estimator WHERE estimator in (amp_users.userId) and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and estimator > 0 group by userId ";
        return $this->db->query($sql)->result_array();
    }

    function graphPendingPriceQuantityDaily($startDate = '',$endDate = ''){
      
            $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,amp_users.name,amp_users.userId FROM `amp_projects` left join amp_users on amp_users.userId = amp_projects.estimator WHERE stageId = 5 and estimator in (amp_users.userId) and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and estimator > 0 group by userId ";
        return $this->db->query($sql)->result_array();
    }

    function graphPendingBudgetsPriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,amp_users.name,amp_users.userId FROM `amp_projects` left join amp_users on amp_users.userId = amp_projects.estimator WHERE stageId = 1 and estimator in (amp_users.userId) and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and estimator > 0 group by userId ";
        return $this->db->query($sql)->result_array();
    }   

    function graph90EstimatorPriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,amp_users.name,amp_users.userId FROM `amp_projects` left join amp_users on amp_users.userId = amp_projects.estimator WHERE stageId = 13 and estimator in (amp_users.userId) and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and estimator > 0 group by userId ";
        return $this->db->query($sql)->result_array();
    }    

    function graphProjectsbyScopePriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as all1Price,
        count(scope) as all1,
        (select count(*) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as abatement, 
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as abatementPrice,
        (select count(*) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as site_demolition,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as site_demolitionPrice,
        (select count(*) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as interior_demolition,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as interior_demolitionPrice,
        (select count(*) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as earthwork,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as earthworkPrice,
        (select count(*) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as other,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate') as otherPrice 
         FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' ";
        return $this->db->query($sql)->result_array();    
    }    

    function graphProjectsWonbyScopePriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as all1Price,
        count(scope) as all1,
        (select count(*) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as abatement, 
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as abatementPrice,
        (select count(*) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as site_demolition,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as site_demolitionPrice,
        (select count(*) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as interior_demolition,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as interior_demolitionPrice,
        (select count(*) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as earthwork,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as earthworkPrice,
        (select count(*) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as other,
        (select (IFNULL(sum(bid_price),0)/1000000) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ) as otherPrice 
         FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 ";        
        return $this->db->query($sql)->result_array();    
    }       

    function graphProjectsLostbyScopePriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT (IFNULL(sum(bid_price),0)/1000) as all1Price,
        count(scope) as all1,
        (select count(*) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as abatement, 
        (select (IFNULL(sum(bid_price),0)/1000) from amp_projects where scope like '%abatement%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as abatementPrice,
        (select count(*) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as site_demolition,
        (select (IFNULL(sum(bid_price),0)/1000) from amp_projects where scope like '%site_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as site_demolitionPrice,
        (select count(*) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as interior_demolition,
        (select (IFNULL(sum(bid_price),0)/1000) from amp_projects where scope like '%interior_demolition%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as interior_demolitionPrice,
        (select count(*) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as earthwork,
        (select (IFNULL(sum(bid_price),0)/1000) from amp_projects where scope like '%earthwork%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as earthworkPrice,
        (select count(*) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as other,
        (select (IFNULL(sum(bid_price),0)/1000) from amp_projects where scope like '%other%' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ) as otherPrice 
         FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 ";
        return $this->db->query($sql)->result_array();  
    }   

    function graphSalesWonbyScopePriceQuantityDaily($startDate = '',$endDate = ''){
        $sql = "SELECT sales,(IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,(select name from amp_users where userId = sales) as name FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and sales > 0 and stageId = 12 group by sales";
        return $this->db->query($sql)->result_array();
    } 

    function graphSalesWonbyProject($startDate = '', $endDate = ''){
        $sql = "SELECT projectId,projectName,bid_price,(select name from amp_users where userId = sales) as name FROM `amp_projects` WHERE  date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 12 order by projectId ASC";

        return $this->db->query($sql)->result_array();
    }       

    function graphEstimatorWonbyScopePriceQuantityDaily($startDate = '',$endDate = ''){
       $sql = "SELECT (IFNULL(sum(bid_price),0)/1000000) as firstsum,count(*) as first,(select name from amp_users where userId = estimator) as name FROM `amp_projects` WHERE date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and estimator > 0 and stageId = 10 group by estimator";
        return $this->db->query($sql)->result_array(); 
    }

    function graphEstimatorWonbyProject($startDate = '', $endDate = ''){
        $sql = "SELECT projectId,projectName,bid_price,(select name from amp_users where userId = estimator) as name FROM `amp_projects` WHERE  date(STR_TO_DATE(dueDate,'%m/%d/%Y')) >= '$startDate' and date(STR_TO_DATE(dueDate,'%m/%d/%Y')) <= '$endDate' and stageId = 10 order by projectId ASC";
